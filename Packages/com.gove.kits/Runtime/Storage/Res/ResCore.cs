...
namespace GoveKits.Runtime.Storage
{
    ...
    public static class ResCore
    {
        ...
        private static async UniTask<bool> InitPackageAsync(PackageConfig config, bool setAsDefault = false)
        {
            ...
            switch (ePlayMode)
            {
                ...
                case EPlayMode.OfflinePlayMode:
                    PrepareOfflinePlayModeOnDevice(config.PackageName);
                    var offlineFileSystem = CreateOfflineBuildinFileSystemParameters();
        ...
        private static FileSystemParameters CreateOfflineBuildinFileSystemParameters()
        {
            var fileSystem = FileSystemParameters.CreateDefaultBuildinFileSystemParameters();
#if !UNITY_EDITOR
            // 真机离线模式不依赖 BuildinCatalog 白名单，避免覆盖安装后清单与缓存 manifest 不一致
            fileSystem.AddParameter(FileSystemParametersDefine.DISABLE_CATALOG_FILE, true);
#endif
            return fileSystem;
        }

        /// <summary>
        /// 清理设备上可能残留的旧版 YooAsset 清单缓存。
        /// 覆盖安装时 CopyBuildinFile 若发现目标文件已存在会直接跳过，导致 manifest 与 APK 内置资源不一致。
        /// </summary>
        private static void PrepareOfflinePlayModeOnDevice(string packageName)
        {
#if !UNITY_EDITOR
            try
            {
                string packageCacheRoot = Path.Combine(Application.persistentDataPath, "yoo", packageName);
                string manifestCache = Path.Combine(packageCacheRoot, "ManifestFiles");
                if (Directory.Exists(manifestCache))
                {
                    Directory.Delete(manifestCache, true);
                    LogCore.Info(nameof(ResCore), $"已清理陈旧清单缓存: {manifestCache}");
                }
            }
            catch (Exception ex)
            {
                LogCore.Warning(nameof(ResCore), $"清理清单缓存失败: {ex.Message}");
            }
#endif
        }
